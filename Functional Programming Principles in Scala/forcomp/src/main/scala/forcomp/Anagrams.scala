...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(ch => ch.toLower).mapValues(_.size).toList.sorted
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y.foldLeft(x.toMap)(subtract).toList.filter(_._2 > 0)
